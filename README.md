...
Forecasts a Formula 1 race **after qualifying, before the start**, from
[FastF1](https://docs.fastf1.dev/) data with LightGBM. The goal is winner and
podium accuracy; full-field ordering is kept as a guardrail. Every feature must
be computable from information available at the forecast cutoff, not merely
from prior races. Those are different guarantees, and the gap between them hid
a real leak (see [Corrections](#corrections-applied)).

**The order is roughly grid-equivalent; the probabilities are not.** Backtested
over 6 expanding-window season folds (127 races), paired against the starting
grid with 95% intervals from 10,000 race-level bootstrap resamples:

- **Order: not distinguishable from the grid at the FIX_PLAN §8 margin.** The
  champion ranker's winner accuracy is 0.6063 against the grid's 0.5591, a
  difference of +0.0472 [−0.0315, +0.1260]. `src.gates` still exits 1 because
  the winner and podium thresholds (+0.05 and +0.03) are not met. Ordering
  quality over finishers does resolve: Spearman +0.0304 [+0.0086, +0.0523].
- **Probabilities: all four probability gates pass.** A Plackett-Luce layer
  over the ranker scores, temperature-calibrated per fold, gives winner log
  loss **1.1469 against the grid baseline's 1.6568**, paired −0.5099 [−0.8659,
  −0.2330]. Podium Brier is 0.07083 vs 0.07108 (the gate allows a worsening of
  up to 0.01), there are 0 coherence problems over 127 races, and log loss
  beats the uniform floor (1.1469 vs 3.0047). Calibration is mildly
  conservative: the favourite's mean `p_win` is 0.5619 against an actual win
  rate of 0.6063.

Read the probabilities as the useful output and the order as roughly
grid-equivalent. `predict.py` prints that distinction on every forecast.

**Two earlier claims from this project did not replicate, for the same reason.**
An earlier README reported, from the 11 races of 2026 alone, that the grid wins
0.7273 and "the machine learning adds nothing." An earlier backtest reported,
from 47 races of 2022–2026, that the blend was significantly *worse* at picking
winners (−0.0851, interval excluding zero). Neither replicates on 186 races.
The cause was the data window, not the method: "the interval excludes zero" is
necessary but not sufficient when the window is narrow.

**Status.** Milestones 0–5 of [`FIX_PLAN.md`](FIX_PLAN.md) complete; milestone
6 measured and mostly negative. 186 races (2018–2026), 267 tests. The reasoning
behind every decision is in [`REASONING.md`](REASONING.md).

## Results

The full tables behind the opening, over test seasons 2021–2026:
...
`src.gates` still exits 1. Volume is no longer the reason: 6 folds and 127
races clear those checks. What fails is the winner and podium thresholds
themselves (+0.05 and +0.03).

### Probabilities: the part that passes

The champion adds a Plackett-Luce layer over the ranker scores,
temperature-calibrated per fold. All four probability gates pass:
...
The bundle refuses to score a race at or before its own cutoff. That is the one
failure that would look like excellent accuracy and be pure hindsight. The
specialist winner/podium head ensemble is a **challenger and is deliberately
excluded**; it scored worse.

### Why the earlier claims did not replicate

Grid winner accuracy is 0.5591 across 127 backtested races; 2026 was simply an
unusually grid-predictable season. Re-running the earlier 47-race rolling
scheme on the full data gives +0.0154 [−0.0231, +0.0538].
...
- **Label semantics.** The inherited `classified = notna(Position)` was true for
  ...
  classification: retirements keep their real places rather than collapsing
  to last.
...
- **Qualifying pace is now used.** Seven per-segment features are live (see
  What was tried for what that did and did not buy).
...
made on the backtest in Results.
...
Evaluate. This is what decides anything, not a single held-out season:
...
to replace the record. That is deliberate: the pre-correction reference cannot
be destroyed by a routine re-run.

`python -m pytest` runs 267 tests.
...
`python -m pytest` runs 267 tests. They target the specific failure modes in
